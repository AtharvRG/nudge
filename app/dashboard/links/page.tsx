...
export default function LinksPage() {
    ...
    const ensureStealthPublicKey = async () => {
        ...
        if (cached) {
            try {
                ...
                setStealthPublicKey(parsed.publicKey);
    ...
    const fetchLinks = async () => {
        if (!user) return;
        setLoading(true);
        // Best-effort cleanup for expired links (moved to server-side or cron, disabling client trigger for now)
    ...
    const handleDelete = async (id: string) => {
        ...
        if (!confirm("Are you sure? This will remove the link but keep transaction history.")) return;

        try {
            // Delete the link (transactions are kept for history)
            // Delete the link using the secure API that handles transactions
            ...
            alert("An error occurred while deleting the blink: " + (err.message || "Unknown error"));
        }
    };
    ...
    return (
        <div className="space-y-8">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-5xl font-display text-white">Your <span className="text-[#F72798]">Blinks</span></h1>
                    <p className="text-zinc-400">Manage your private payment links.</p>
                </div>

                <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                    <DialogTrigger asChild>
                        <Button className="bg-[#F72798] hover:bg-white text-xl text-black hover:text-[#F72798] rounded-xl h-12 px-6 border-0 shadow-[0_0_20px_rgba(247,39,152,0.3)] transition-all hover:scale-[1.05] active:scale-[0.95]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {links.map((link) => (
                        <div key={link.id} className="bg-zinc-900 border border-white/5 p-6 rounded-2xl group hover:border-neon-pink/50 transition-colors">
                            ...
                            <h3 className="text-3xl font-display text-white mb-1">{link.title}</h3>
