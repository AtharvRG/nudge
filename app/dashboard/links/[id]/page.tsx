...
export default function LinkDetailsPage() {
    ...
    const loadData = async () => {
        ...
        if (new Date(linkData.created_at) < oneWeekAgo) {
            // Use API to delete securely and handle transactions
            await fetch(`/api/links/${linkData.id}`, { method: 'DELETE' });
            router.push("/dashboard/links");
            return;
        }
        setLink(linkData);
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="ml-auto flex gap-2">
                    ...
                    <Button
                        variant="ghost"
                        className="text-red-500 hover:bg-red-500/20 border-none"
                        onClick={async () => {
                            if (confirm('Are you sure you want to delete this link?')) {
                                setLink(null); // Optimistic update / hide
                                try {
