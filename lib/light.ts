...
export async function getShieldedBalance(
    ...
    // If client-side, use the API proxy to avoid CORS/Browser network issues
    if (typeof window !== "undefined") {
        try {
            const response = await fetch("/api/light/balance", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ publicKey: stealthPublicKey.toBase58() }),
            });

            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.details || data.error || "Failed to fetch balance via proxy");
            }

            const data = await response.json();
            return data.balance;
        } catch (error) {
            // Fallthrogh to direct RPC call if proxy fails? 
            // Better to throw here if we know direct call fails.
            throw new RpcConnectionError(error instanceof Error ? error : undefined);
        }
    }

    // Server-side (or fallback), use direct RPC
    try {
...
export async function buildUnshieldTx(
    ...
    try {
        ...
        // =====================================================================
        // STEP 1-3 (Proxy Mode): Fetch Data & Proofs via Server API
        // =====================================================================
        let selectedAccounts: CompressedAccountWithMerkleContext[] = [];
        let totalLamports = new BN(0);
        let rootIndices: number[] = [];
        let compressedProof: any;

        if (typeof window !== "undefined") {
            // BROWSER MODE: Use Proxy to avoid CORS
            const response = await fetch("/api/light/unshield-data", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    stealthPublicKey: stealthKeypair.publicKey.toBase58(),
                    recipientPublicKey: recipientMainWallet.toBase58(),
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || "Failed to fetch unshield data via proxy");
            }
            ...
            rootIndices = data.rootIndices;
            compressedProof = {
                a: new Uint8Array(Object.values(data.compressedProof.a)),
                b: new Uint8Array(Object.values(data.compressedProof.b)),
                c: new Uint8Array(Object.values(data.compressedProof.c))
            };

        } else {
            // SERVER/CLI MODE: Direct RPC Calls
            const result = await withTimeout(
            ...
            if (allAccounts.length === 0) throw new NoShieldedFundsError();
